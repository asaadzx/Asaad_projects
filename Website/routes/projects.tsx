      ...
      <div className="page-container min-h-screen bg-gradient-to-b from-blue-900 via-purple-900 to-indigo-900 text-white"> {/* Added class for better styling and semantic meaning */}
        <header class="site-header container mx-auto px-4 py-8"> {/* Added class for better styling and semantic meaning */}
          <nav class="nav-container flex justify-between items-center"> {/* Added class for better styling and semantic meaning */}
            ...
            <div class="nav-links flex gap-6"> {/* Added class for better styling and semantic meaning */}
              <a href="/" class="nav-link hover:text-blue-300 transition-colors">Home</a>
              <a href="/about" class="nav-link hover:text-blue-300 transition-colors">About</a>
              <a href="/projects" class="nav-link text-blue-300 font-medium">Projects</a>
            </div>
          </nav>
        </header>

        <main class="projects-main container mx-auto px-4 py-8"> {/* Added class for better styling and semantic meaning */}
          <h1 class="main-title text-4xl font-bold mb-2 text-center animate__animated animate__fadeInDown">My Projects</h1>
          <p class="main-description text-xl text-center mb-12 animate__animated animate__fadeIn">Check out what I've been working on!</p>

          <div class="project-grid grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-12"> {/* Added class for better styling and semantic meaning */}
            {projects.map((project, index) => (
              <div key={project.title} class={`project-card bg-white/10 backdrop-blur-md rounded-xl overflow-hidden shadow-xl border border-white/20 hover:transform hover:scale-105 transition-all duration-300 hover:shadow-2xl animate__animated animate__fadeInUp animate__delay-${index % 3}s`}> {/* Added class for better styling and semantic meaning */}
                <div class="project-image h-48 bg-gray-800 flex items-center justify-center overflow-hidden"> {/* Added class for better styling and semantic meaning */}
                  <img 
                    src={project.image} 
                    alt={project.title} 
                    class="w-full h-full object-cover" {/* Added class for better styling and semantic meaning */}
                    onError={(e) => {
                ...
                <div class="project-details p-6"> {/* Added class for better styling and semantic meaning */}
                  <h3 class="project-title text-xl font-bold mb-2">{project.title}</h3>
                  <p class="project-description mb-4 text-gray-300">{project.description}</p>
                  <div class="project-tags flex flex-wrap gap-2 mb-4"> {/* Added class for better styling and semantic meaning */}
                    {project.tags.map(tag => (
                      <span class="tag px-2 py-1 bg-white/20 rounded-full text-xs">{tag}</span> {/* Added class for better styling and semantic meaning */}
                    ))}
                  </div>
                  <div class="project-links flex justify-between"> {/* Added class for better styling and semantic meaning */}
                    <a href={project.link} class="project-link text-blue-400 hover:text-blue-300 transition-colors flex items-center gap-1">
                      <span>Demo</span>
                    ...
                    <a href={project.github} class="project-link text-blue-400 hover:text-blue-300 transition-colors flex items-center gap-1">
                      <span>Code</span>
